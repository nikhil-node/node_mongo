...
//Middleware to restrict user action based on role,
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(new AppError(`${req.user.role}`, 401));
    }
    next();
  };
//Forgot password
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError("Request user not found", 404));
  }
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Your reset password valid till 10 min",
      message,
    });
    ...
    return next(new AppError("There was something wrong!", 500));
  }
});
//Reset password
